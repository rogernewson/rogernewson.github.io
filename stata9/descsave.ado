...
syntax [anything(id="varlist")] [using] [, DOfile(string asis)
 ...
 SImple Short Detail Fullnames Numbers
 ];
/*
 ...
 The options simple, short, detail, fullnames and numbers
   correspond to options of the same names for the describe command,
   and are added for backwards compatibility (and ignored),
   because the Stata 8 version of descsave passed them to describe to get the listing,
...
* Call describe to set returned results in r() *;
qui describe `varlist', simple;
