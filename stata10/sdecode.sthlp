...
{phang}
{cmd:replace} specifies that the output string variable will replace the input
...
{cmd:none}, {cmd:x10}, {cmd:rtfsuper}, {cmd:texsuper} and {cmd:htmlsuper}.
...
{pstd}
If the user specifies an {cmd:esub()} option, then {cmd:sdecode} performs exponent substitution
...
{psee}
Manual:  {hi:[D] compress}, {hi:[D] destring}, {hi:[D] encode}, {hi:[D] format}, {hi:[D] functions}, {hi:[D] generate}, {hi:[D] label}
{p_end}
...
{break}{helpb sencode} if installed
{p_end}
