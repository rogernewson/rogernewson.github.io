...
if(_rc==0){;
  qui encode `by' if(`touse'), gene(`tempgroup1');
  lab val `tempgroup1';
};
else{;
  qui gene double `tempgroup1'=`by' if(`touse');
};
tempname oldest;
cap esti hold `oldest';
qui somersd `tempgroup1' `yvar' [`weight'`exp'] if `touse',
...
twogp `by' if(`touse') [`weight'`exp'], cfweight(`cfweight') gene(`group1');
...
syntax varname [if] [in] [fweight aweight iweight pweight]
  , Generate(string) [ CFWeight(string asis) ];
local by "`varlist'";
...
*
 ...
 containing value of weight expression to be passed to summarize and tabulate.
*;
tempname tswexp;
if("`weight'"=="fweight"){;
  ...
  qui gene double `tswexp'=1;
};
if `"`cfweight'"'!="" {;
  qui replace `tswexp' = `tswexp' * ( `cfweight' ) if `touse';;
};
qui compress `tswexp';
qui replace `touse'=0 if missing(`tswexp') | (`tswexp'==0);
