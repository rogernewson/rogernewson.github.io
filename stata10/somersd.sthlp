...
{title:Somers' D or Kendall's tau-a with confidence intervals}
...
{pstd}
{opt bootstrap}, {opt by}, {opt jackknife}, {opt statsby},
...
{pstd}
The module {cmd:somersd} calculates values of Somers' D or Kendall's tau-a
...
differences between the population Somers' D or tau-a values.
...
absent, then Somers' D is calculated.
...
then the standardized Somers' D estimates are assumed to be sampled from a standard Normal distribution.
...
{p 4 8 2}
{cmd:funtype(}{it:functional_type}{cmd:)} specifies whether the Somers' D
...
coefficient is a special case of the clustered Von Mises Somers' D.  For
...
{cmd:somersd} estimates stratified Somers' D or Kendall's tau-a
...
{cmd:bstrata()} is specified, then {cmd:somersd} estimates Somers' D or
...
Somers' D, Kendall's tau-a or Greiner's rho parameters. If {cmd:transf()}
...
{pstd}
In the case of a binary X-variable, Somers' D is the parameter
...
Somers' D is related to the area A under the ROC curve by the
formula
...
are described in detail in Newson (2006a), Newson (2006b) and Newson (2006c).
...
in the conventional terminology for defining Somers' D.
...
the Somers' D or Kendall's tau-a statistics.
