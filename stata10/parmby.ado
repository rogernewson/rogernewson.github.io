...
string scalar tfcur,lcquote,rcquote,groupvar,byvars,command,parmestopts
real matrix groupview,byrange
real scalar groupseq,retcode
/*
...
/*
  Initialize constant string scalars
*/
lcquote="`"+`"""'
rcquote=`"""'+"'"
...
for(groupseq=1;groupseq<=rows(byrange);groupseq++) {
  tfcur=st_local("df0")
  stata(
    "qui use in " + strofreal(byrange[groupseq,1]) + "/" + strofreal(byrange[groupseq,2])
  ...
  stata("qui gene long " + groupvar + "=" + strofreal(groupseq))
  stata("qui compress `group'")
  tfcur=st_local("tf" + strofreal(groupseq))
