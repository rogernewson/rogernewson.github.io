...
syntax [, Label YLabel EForm Dof(real -1) LEvel(numlist >=0 <100 sort) CLNumber(string)
    SAving(string asis) noREstore FAST
    EMac(string asis) EScal(string asis) EVec(string asis)
    IDNum(string) IDStr(string) FList(string)
    REName(string) FOrmat(string) LIst(string) noDOUble FLOAT ];
/*
 ...
 The user must specify at least one of the three options
 saving, norestore and fast,
 because they specify whether the output data set is saved to a disk file,
 written to the memory (destroying any pre-existing data set), or both.
