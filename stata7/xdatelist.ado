...
if(`firstd'==.){disp as error "Invalid first(`first')";};
if(`lastd'==.){disp as error "Invalid last(`last')";};
if((`firstd'==.)|(`lastd'==.)){error 498;};
...
if("`unit'"=="day"){
  while(`date'<=`lastd'){
    *
     This comment seems to be required to stop Stata getting confused
     about nested braces (for no apparent reason)
    *;
