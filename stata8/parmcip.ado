...
if "`eform'"=="" {;
  ...
  qui gene `maxntype' `sestimate' = log(`estimate') if `touse';
  qui gene `maxntype' `sstderr' = `stderr' / `estimate' if `touse';
};


* Add t-statistics or z-scores and P-values *;
if "`tdist'"=="notdist" {;
    qui gene `maxntype' `zstat' = `sestimate' / `sstderr' if `touse';
    qui gene `maxntype' `pvalue' = 2 * normprob(-abs(`zstat')) if `touse';
    qui compress `tstat' `pvalue';
    label variable `zstat' "Standard normal deviate";
    label variable `pvalue' "P-value";
};
else {;
    qui gene `maxntype' `tstat' = `sestimate' / `sstderr' if `touse';
    qui gene `maxntype' `pvalue' = tprob(`dof',`tstat') if `touse';
...
foreach leveli1 of numlist `level' {;
    ...
    if "`tdist'"=="notdist" {;
        qui gene `maxntype' `hwid'= `sstderr'*invnorm(1-(100-`leveli1')/200) if `touse';
    };
    else {;
        qui gene `maxntype' `hwid' = `sstderr'*invttail(`dof',(100-`leveli1')/200) if `touse';
    };
    qui gene `maxntype' `cimin' = `sestimate' - `hwid' if `touse';
    qui gene `maxntype' `cimax' = `sestimate' + `hwid' if `touse';
