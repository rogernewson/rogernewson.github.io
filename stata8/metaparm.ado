...
NUDNum() specifies a name for the numeric ID variable (defaulting to -idnum-).
...
*
 ...
 and Satterthwaite denominators
*;
qui {;
...
* Recast, label and format summarized variables *;
if ("`sumvar'" != "") & ("`maxntype'" != "double") {;
  recast `maxntype' `sumvar', force;
};
forv i1=1(1)`nsumvar' {;
  local sumvcur: word `i1' of `sumvar';
...
qui {;
  gene `maxntype' `estimate' = `westi';
  gene `maxntype' `stderr' = sqrt(`w2vari');
  if "`eform'"!="" {;
  ...
  format `stderr' `sefmt';
  if "`tdist'"=="tdist" {;
    gene `maxntype' `dof' = (`w2vari' * `w2vari') / `satden';
    compress `dof';
    lab var `dof' `"`doflab'"';
    format `dof' `doffmt';
  };
};
drop `westi' `w2vari' `satden';
...
*;
if "`tdist'"=="notdist" {;
  parmcip, estimate(`estimate') stderr(`stderr') `tdist' `eform' `float' `options';

};
else {;
  parmcip, estimate(`estimate') stderr(`stderr') dof(`dof') `tdist' `eform' `float' `options';
};
