...
*
 Add -plot()- option to subcommand list if necessary
*;
if `"`plot'"'!="" {;
...
if "`horizontal'"=="horizontal" {;
  local twowayopts `"yscale(reverse) ylabel(, valuelabel angle(0)) ytitle(`"`parmlab'"') xtitle(`"`estlab'"') `deflegend' `twoptions'"';
};
else {;
  local twowayopts `" xlabel(, valuelabel) ytitle(`"`estlab'"') xtitle(`"`parmlab'"') `deflegend' `twoptions'"';
};
